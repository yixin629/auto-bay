"""Xiaohongshu (RED) connector (v1 scaffold).

Provides protocol-compliant behavior and credential validation so RED can be
...
@ConnectorRegistry.register(Platform.XIAOHONGSHU)
class XiaohongshuConnector:
    """Xiaohongshu connector with safe fallback behavior."""
    ...
    def __init__(self, credentials: dict, region: str):
        ...
        # Compatibility: some SDK docs use client_id/client_secret naming.
        self._app_id = self.credentials.get("app_id") or self.credentials.get("client_id", "")
        self._app_secret = self.credentials.get("app_secret") or self.credentials.get("client_secret", "")
    ...
    def _parse_order(self, raw: dict) -> ExternalOrderData:
        ...
        receiver = raw.get("receiver") or raw.get("address") or raw.get("shipping_address") or {}
        payment = raw.get("payment") or raw.get("amount") or {}
        ...
        for item in raw_items:
            ...
            unit_price = self._to_decimal(item.get("unit_price") or item.get("price") or "0")
            subtotal += unit_price * quantity
            line_items.append(
                {
                    "title": item.get("title") or item.get("product_name") or "Unknown",
                    "sku": item.get("sku") or item.get("sku_id") or "",
                    "quantity": quantity,
                    "unit_price": str(unit_price),
                }
            )

        shipping_cost = self._to_decimal(payment.get("shipping_fee") or raw.get("shipping_fee") or "0")
        tax = self._to_decimal(payment.get("tax") or raw.get("tax") or "0")
        total = self._to_decimal(payment.get("total") or raw.get("total_amount") or subtotal + shipping_cost + tax)
        ...
        return ExternalOrderData(
            ...
            tax=tax,
            total=total,
            currency=str(raw.get("currency") or payment.get("currency") or "CNY"),
            ordered_at=self._to_datetime(raw.get("created_at") or raw.get("create_time") or raw.get("order_time")),
        )
    ...
    async def create_listing(self, data: ListingCreateDTO) -> ExternalListingResult:
        return ExternalListingResult(
            external_id=f"xiaohongshu-{data.internal_product_id}",
            status=self._status(),
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                for _ in range(max_pages):
                    ...
                    resp = await client.get(
                        "https://ark.xiaohongshu.com/open_api/v3/order/list",
                        params=params,
                        headers=headers,
                    )
    ...
    async def refresh_credentials(self) -> dict:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                resp = await client.post(settings.xiaohongshu_oauth_token_url, data=payload)
                resp.raise_for_status()
